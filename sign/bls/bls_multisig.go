...
//
// The function returns:
//   - (nil, errBLSAggregateEmptyList) if no signatures are provided (input slice is empty)
//   - (nil, errInvalidSignature) if a deserialization of at least one signature fails (input is an invalid serialization of a
...
func AggregateBLSSignatures(sigs []sign.Signature) (sign.Signature, error) {
	...
	for i, sig := range sigs {
		if len(sig) != SignatureLenBLSBLS12381 {
			return nil, fmt.Errorf("signature at index %d has an invalid length: %w", i, errInvalidSignature)
		}
...
func BatchVerifyBLSSignaturesOneMessage(
	...
	// empty list check
	if len(pks) == 0 {
		return falseSlice, fmt.Errorf("invalid list of public keys: %w", errBLSAggregateEmptyList)
	}
	...
	for i, pk := range pks {
		pkBLS, ok := pk.(*pubKeyBLSBLS12381)
		if !ok {
			return falseSlice, fmt.Errorf("key at index %d is invalid: %w", i, errNotBLSKey)
		}
...
// errBLSAggregateEmptyList is returned when a list of BLS objects (e.g. signatures or keys)
